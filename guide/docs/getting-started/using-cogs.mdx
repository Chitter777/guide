...
Тот код, который мы написали, будет частью [**Начальных файлов**](./initial-files.mdx), 
и мы предпологаем, что там уже существует файл `main.py` в вашем каталоге, который иницирует экземпляр `commands.Bot()`.
